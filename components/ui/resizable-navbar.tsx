...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn("sticky inset-x-0 top-5 sm:top-20 z-40 w-full", className)}
    >
...
export const NavItems = ({ items, className, onItemClick, activePath }: NavItemsProps) => {
  ...
  const isActive = (link: string) =>
    link.startsWith("/") && !link.startsWith("/#") && activePath === link;
    ...
    >
      {items.map((item, idx) => (
        ...
        >
          ...
          {isActive(item.link) && (
            <span className="absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 w-4 rounded-full bg-white/60" />
          )}
        </a>
      ))}
    </motion.div>
...
export const NavbarLogo = () => {
  return (
    <a href="/" className="relative z-20 mr-4 px-2 py-1">
      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#1a1a1a] border border-white/10 overflow-hidden">
